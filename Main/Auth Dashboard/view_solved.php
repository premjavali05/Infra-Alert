<?php
  ...
  $sql1 = "SELECT * FROM complaint WHERE category = '$category' AND dept = '$dept' AND state = '$state' AND city = '$city' AND status = 'Solved' ";
  $result1 = mysqli_query($connect,$sql1);

  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $eid = $_POST['eid'];
    $cid = $_POST['cid'];
    header("Location: solved_filter.php?eid=$eid&cid=$cid");
    exit;
    }
        ...
        <tbody>
            <?php
                ...
                while ($fetch = mysqli_fetch_array($result1))
                {
                    ...
                    echo "<td>" . '<a href = "../Auth Dashboard/solved_pdf.php?phno='.$fetch['phno']. '&' .'img_url='.$fetch['img_url'].'">' . '<button><i class="ri-link"></i></button>' . "</td>";
                    echo "</tr>";
                   $sno = $sno + 1;
                }
            ?>
